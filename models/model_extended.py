...
def hhp_net(mean, std, alpha=1):
    ...
    input_x = tf.keras.layers.Lambda(lambda k: tf.cast(k[:, 0:33:3], tf.float32), input_shape=(33, 1))(input)
    ...
    input_y = tf.keras.layers.Lambda(lambda k: tf.cast(k[:, 1:33:3], tf.float32), input_shape=(33, 1))(input)
    ...
    input_c = tf.keras.layers.Lambda(lambda k: tf.cast(1 / std * (k[:, 2:33:3] - mean), tf.float32), input_shape=(33, 1))(input)
