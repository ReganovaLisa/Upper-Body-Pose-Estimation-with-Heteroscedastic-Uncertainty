...
def hhp_net(mean, std, alpha=1):
    ...
    """
    input = tf.keras.layers.Input(shape=(15, 1), name="input")

    num_filters = 5

    input_x = tf.keras.layers.Lambda(lambda k: tf.cast(k[:, 0:15:3], tf.float32), input_shape=(15, 1))(input)
    ...
    input_y = tf.keras.layers.Lambda(lambda k: tf.cast(k[:, 1:15:3], tf.float32), input_shape=(15, 1))(input)
    ...
    input_c = tf.keras.layers.Lambda(lambda k: tf.cast(1 / std * (k[:, 2:15:3] - mean), tf.float32), input_shape=(15, 1))(input)
    ...
    c1 = tf.keras.activations.sigmoid(input_c)
    ...
    # yaw.shape = (None, 2) -> angle and uncertainty

    return model_exp



def hhp_net_all_points(mean, std, a, alpha=1):
    """
    Create a model based on the experiment parameter passed as input
    ...
    input = tf.keras.layers.Input(shape=(a, 1), name="input")

    num_filters = int(a/3)

    input_x = tf.keras.layers.Lambda(lambda k: tf.cast(k[:, 0:a:3], tf.float32), input_shape=(a, 1))(input)
    model_x = tf.keras.layers.Conv1D(filters=num_filters, kernel_size=1, strides=1, use_bias=True, kernel_initializer='random_normal')(input_x)
    x1 = tf.keras.layers.LeakyReLU(0.1)(model_x)
    x1 = tf.keras.layers.Flatten()(x1)

    input_y = tf.keras.layers.Lambda(lambda k: tf.cast(k[:, 1:a:3], tf.float32), input_shape=(a, 1))(input)
    model_y = tf.keras.layers.Conv1D(filters=num_filters, kernel_size=1, strides=1, use_bias=True, kernel_initializer='random_normal')(input_y)
    y1 = tf.keras.layers.LeakyReLU(0.1)(model_y)
    y1 = tf.keras.layers.Flatten()(y1)

    # input_c = tf.keras.layers.Lambda(lambda k: tf.cast(1 / std * (k[:, 10:15] - mean), tf.float32), input_shape=(15, 1))(input)
    input_c = tf.keras.layers.Lambda(lambda k: tf.cast(1 / std * (k[:, 2:a:3] - mean), tf.float32), input_shape=(a, 1))(input)
    input_c = tf.keras.layers.Conv1D(filters=num_filters, kernel_size=1, strides=1, use_bias=False, kernel_initializer='random_normal')(input_c)
    c1 = tf.keras.activations.sigmoid(input_c)
    c1 = tf.keras.layers.Flatten()(c1)
...
def hhp_net_all_points_ext(mean, std, a, alpha=1):
    """
    Create a model based on the experiment parameter passed as input
    ...
    d0 = tf.keras.layers.Dense(200 * alpha)(xy_merge)
    d0 = tf.keras.layers.LeakyReLU(0.1)(d0)
    d0 = tf.keras.layers.Dropout(rate = 0.2)(d0)
    
    d0 = tf.keras.layers.Dense(250 * alpha)(d0)
    d0 = tf.keras.layers.LeakyReLU(0.1)(d0)
    d0 = tf.keras.layers.Dropout(rate = 0.2)(d0)

    d0 = tf.keras.layers.Dense(300 * alpha)(d0)
    d0 = tf.keras.layers.LeakyReLU(0.1)(d0)
    d0 = tf.keras.layers.Dropout(rate = 0.2)(d0)
    ...
    d0 = tf.keras.layers.Dense(200 * alpha)(d0)
    d0 = tf.keras.layers.LeakyReLU(0.1)(d0)
    d0 = tf.keras.layers.Dropout(rate = 0.2)(d0)

    d0 = tf.keras.layers.Dense(150 * alpha)(d0)
    d0 = tf.keras.layers.LeakyReLU(0.1)(d0)
    d0 = tf.keras.layers.Dropout(rate = 0.2)(d0) 

    d0 = tf.keras.layers.Dense(100 * alpha)(d0)
    d0 = tf.keras.layers.LeakyReLU(0.1)(d0)
    d0 = tf.keras.layers.Dropout(rate = 0.2)(d0)

    d0 = tf.keras.layers.Dense(50 * alpha)(d0)
    d0 = tf.keras.layers.LeakyReLU(0.1)(d0)
...
def hhp_net_extended(mean, std, alpha=1):
    """
    Create a model based on the experiment parameter passed as input

    11 keypoints are passed as input = 11*3 =33
    https://google.github.io/mediapipe/solutions/pose#python-solution-api

    Maybe extended to support https://google.github.io/mediapipe/solutions/face_mesh#python-solution-api
    ...
    Returns:
        :model_exp (tf.keras.models.Model): the model built accordingly to the experiment number
    """
    input = tf.keras.layers.Input(shape=(33, 1), name="input")
    ...
    d0 = tf.keras.layers.Dense(250 * alpha, kernel_initializer='random_normal')(xy_merge) #250
    d0 = tf.keras.layers.LeakyReLU(0.1)(d0)

    d0 = tf.keras.layers.Dense(200 * alpha, kernel_initializer='random_normal')(d0) #200
    d0 = tf.keras.layers.LeakyReLU(0.1)(d0)

    d0 = tf.keras.layers.Dense(150* alpha, kernel_initializer='random_normal')(d0) #150
    d0 = tf.keras.layers.LeakyReLU(0.1)(d0)
